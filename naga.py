...
class HostConn(Connection):
    '''Extends fabric Connection class to include friendly name and persistent attributes'''
...
def get_sudo():
    ...
    return Config(overrides={'sudo': {'password': getpass("What's your sudo password? ")}})

def file_flag_check(conn, directory, flag):
    '''Check for file-based flags like .nopull'''
    if conn.run(f'ls -al {directory} | grep {flag}', hide=True, warn=True).stdout.splitlines():
        return True
    else:
        return False
...
def git_repo(conn, directory):
    ...
    if "working tree clean" not in status[-1]:
        ...
        update = conn.run(f'cd {directory} && git pull', hide=True).stdout.splitlines()
